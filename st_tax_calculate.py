...
def main():
    ...
    with st.sidebar:
        ...
        with col1:
            monthly_salary = st.number_input("月薪", min_value=0, value=15000, step=1000)
            
            social_base = st.number_input("社保基数", min_value=0, value=4812, step=100)
            social_base_ratio= st.number_input("社保比例(%)", min_value=5.0, max_value=12.0, value=10.5, step=0.5)
            special_deduction = st.number_input("专项附加扣除", min_value=0, value=1500, step=500)
        with col2:
            annual_bonus = st.number_input("年终奖", min_value=0, value=45000, step=5000)
    ...
    if calculate_clicked:
        ...
        with st.spinner("正在计算，请稍候..."):
            ...
            # 月度收入图表
            axes[1, 0].plot(df['公积金基数'], df['平均月度现金收入(不含公积金)'], 'b-', linewidth=2, marker='o', markersize=4, label='平均月度现金收入(不含公积金)')
            ...
            if best['公积金基数'] > company_base:
                ...
                - 虽然每月现金收入减少 **{base_case['平均月度现金收入(不含公积金)'] - best['平均月度现金收入(不含公积金)']:,.0f} 元**
